...
class DataMiner():
    def __init__(self, start_version:str, end_version:str, **kwargs) -> None:
        ...
        if start_version == "-": start_version = Manifest.get_order_id_version(0)
        self.start_id = Manifest.get_order_version_id(start_version)
        self.end_version = end_version
        if end_version == "-": end_version = Manifest.get_latest()[1]
        self.end_id = Manifest.get_order_version_id(end_version)
        if kwargs is None: kwargs = {}
        self.init(**kwargs)
    
    def is_valid_version(self, version:str) -> bool:
        '''Returns if the given version is within the dataminer's range.'''
        version_id = Manifest.get_order_version_id(version)
