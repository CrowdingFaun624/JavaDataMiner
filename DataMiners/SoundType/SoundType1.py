...
class SoundType1(DataMiner.DataMiner):
    def search(self, version:str) -> str:
        ...
        if len(sound_type_files) > 1:
            ...
            raise FileNotFoundError("No SoundType file found for %s (SoundEvents.java is \"sound_events_name\")!" % (version, sound_events_name))
    ...
    def activate(self, version:str, store:bool=True) -> dict[str,dict[str,int|str]]:
        ...
        if not isinstance(sound_events, dict): return TypeError("SoundEvents subprocess of SoundType did not give code keys in %s!" % version)
