...
class SoundType4(DataMiner.DataMiner):
    ...
    def search(self, version:str) -> str:
        '''Returns the path of Blocks.java (e.g. "aju.java")'''
        search_terms = [["stone", "grass", "leaves", "dispenser"], ["stone", "grass", "leaves"]][self.search_mode]
        blocks_files = Searcher.search(version, "client", search_terms, ["and"])
    ...
    def activate(self, version:str, store:bool=True) -> dict[str,dict[str,int|str]]:
        ...
        sound_events = SoundEvents.SoundEvents.get_data_file(version) if self.rely_on_sounds else []
