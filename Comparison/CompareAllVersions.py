...
def compare_all(data_type:str) -> None:
    ...
    for old_version in version_list:
        ...
        elif old_data is None and new_data is not None:
            old_data = type(new_data)()
...
def main() -> None:
    ...
    data_types = list(DataMiners.all_dataminers.keys())
    options = data_types.copy()
    options.append("all") # "all" option
    data_type = get_valid_input("Data type (%s): ", options)
    if data_type == "all":
        for data_type in data_types:
            compare_all(data_type)
    else:
        compare_all(data_type)
